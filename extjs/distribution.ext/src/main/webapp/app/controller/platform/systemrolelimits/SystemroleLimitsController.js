Ext.define('app.controller.platform.systemrolelimits.SystemroleLimitsController',{
	extend: 'Ext.app.ViewController',
	alias: 'controller.systemroleLimits',
	init:function(){
    ...
    gridclick: function(gridpanel, record, tr, rowIndex, e, eOpts){
    	var me = this;
    	if(record){
    		me.treePanel.setTitle("系统模块『"+record.data.rolename+"』");
    		me.treePanel.getStore().reload({params:{roleid:record.data.roleid}});
    		me.treePanel.roleid = record.data.roleid;
    	}
    },
    ...
    onSearch:function(){
        ...
        var url = "platform/systemrolelimits/getsystemrole.do";
        me.gridpanel.getStore().reload({params:{roleCode:encodeURIComponent(text)}});
    ...
    saveLimit:function(btn, e){
    	...
    	var roleid = me.treePanel.roleid;
    	...
    	EU.showMsg({title:'提示信息',message:"确定保存当前配置权限吗？",option:1,callback:function(rt){
	    	...
	    	Ext.each(treeNodes,function(rec){
	    		var data = rec.data;
	    		list.push({roleid:roleid,menuid:data.param1,operatecode:data.param2});
	    	});
	    	var url = "platform/systemrolelimits/savelimit.do";
	    	EU.RS({url:url,scope:this,msg:false,params:{list:list,roleid:roleid},callback:function(result){
	         	EU.toastInfo("【保存成功】");
